...
if __name__ == "__main__":
  no_of_rows = int(sys.argv[1]) if len(sys.argv) > 1 else 1000
  df = generate_database(no_of_rows=no_of_rows)
  print("Generated DataFrame:")
  print(df.head())
  engine = create_engine("postgresql://pgvector:pgvector@localhost:5432/pgvector")
  res = df.to_sql("person_data", engine, if_exists="append", index=False)
  if res != no_of_rows:
      print("Failed to insert all rows into the database.")
  else:
      print(f"Successfully inserted {res} rows into the database.")